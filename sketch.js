...
var moneyImg, money;
var sackImg, sack;
var Ctreasure, CtreasureImg, OtreasureImg;
var notes, notesImg;
...
function preload(){
  ...
  OtreasureImg = loadImage("opentreasure.png");
...
function setup() {
  ...
  notes.scale = 0.1;
  notes.setCollider("rectangle", 0,0, 40,40);
...
function draw() {
 ...
 if(keyWentDown(DOWN_ARROW)){
  ...
  /*if(thief.collide(key)){
    key.destroy();
    treasure.changeImage("opentreasure.png",OtreasureImg)
  }*/
  ...
  thief.isTouching(door7)||thief.isTouching(door8)){
  thief.collide(door1) 
  thief.collide(door2);
  thief.collide(door3);
  thief.collide(door4);
...
function timer(){
  fill(0);
  textSize(45);
  text("Timer: "+ setInterval, 30,40);
  
  var timer = setInterval(function(){
    document.getElementById('safeTimerDisplay').innerHTML='60:'+sec;
    sec--;
    if (sec < 0) {
        clearInterval(timer);
    }
